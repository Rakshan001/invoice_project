...
require_once 'includes/amount_to_words.php';  // Include the amount to words conversion
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_invoice'])) {
    try {
        ...
        // Get client details only if with_client is selected
        if ($_POST['invoice_type'] === 'with_client' && !empty($_POST['client_id'])) {
        ...
        // Insert into invoice table with nullable client fields
        $stmt = $conn->prepare("
            INSERT INTO invoice (
        ...
        // Set Content-Type header to ensure it's interpreted as JSON
...
// Alter invoice table to allow null values for client fields if not already done
try {
    $alter_queries = [
        "ALTER TABLE invoice MODIFY client_id INT NULL",
        "ALTER TABLE invoice MODIFY client_name VARCHAR(255) NULL",
        "ALTER TABLE invoice MODIFY client_address TEXT NULL",
        "ALTER TABLE invoice MODIFY client_gstin VARCHAR(50) NULL",
        "ALTER TABLE invoice MODIFY client_state VARCHAR(100) NULL"
    ];
    ...
    // Ignore errors as the columns might already be nullable
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_new_client'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM client_master WHERE company_id = ? AND name = ?");
    $stmt->bind_param("is", $company['company_id'], $name);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    if ($existing_client) {
        $_SESSION['error'] = "Client with this name already exists. Please use a different name.";
    ...
    if ($success) {
        ...
        header("Location: create_invoice.php?client_saved=1&client_id=" . $client_id);
        exit();
    } else {
                    ...
                    <form id="invoiceForm" method="POST" action="">
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-12">
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="invoice_type" id="withClient" value="with_client" checked>
                                    <label class="form-check-label" for="withClient">With Client</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="invoice_type" id="withoutClient" value="without_client">
                                    <label class="form-check-label" for="withoutClient">Without Client</label>
                                </div>
                            </div>
                        </div>

                        <!-- Client Selection (initially visible) -->
                        <div id="clientSection">
                            ...
                            <div class="row g-3 mb-4">
                                <div class="col-md-12">
                                    <div class="form-floating">
                                        <input type="text" class="form-control" id="clientName" name="client_name" placeholder="Start typing client name...">
                                ...
                                <div class="col-md-12">
                                    <div class="form-floating">
                                        <textarea class="form-control" id="clientAddress" name="client_address" style="height: 100px"></textarea>
                                        <label for="clientAddress">Client Address</label>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <input type="text" class="form-control" id="clientState" name="client_state">
                            ...
                            <table class="table table-bordered" id="invoiceItems">
                                <thead class="table-light">
                                    <tr>
                                        ...
                                        <th style="width: 30%; min-width: 250px;">Product Description</th>
                                        <th style="width: 15%; min-width: 120px;">Amount<br>(Rs.)</th>
                                        <th style="width: 15%; min-width: 120px;">Taxable Value<br>(Rs.)</th>
                                        <th style="width: 10%; min-width: 100px;">Rate</th>
                                        <th style="width: 12%; min-width: 120px;">GST Amount<br>(Rs.)</th>
                                        <th style="width: 10%; min-width: 100px;">Total<br>(Rs.)</th>
                                        <th style="width: 3%; min-width: 50px;">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr class="item-row">
                                        ...
                                        <td>
                                            <select class="form-select form-select-sm tax-rate" name="items[0][tax_rate]" style="min-width: 85px;">
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-12">
                                <div class="form-floating">
                                    <input type="text" class="form-control" id="amountInWords" name="amount_in_words" placeholder="Amount in words" readonly>
                                    <label for="amountInWords">Amount in Words (Auto-generated)</label>
                                </div>
                            </div>
                        </div>
...
/* Rest of the existing table styles */
#invoiceItems {
    table-layout: fixed;
    width: 100%;
}
...
#invoiceItems .tax-rate {
    min-width: 85px;
    width: 100%;
}

#invoiceItems .delete-row {
    padding: 0.25rem 0.5rem;
    line-height: 1;
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculateTotals() {
        ...
        if (grandTotalInput) {
            ...
            // Update amount in words whenever grand total changes
            updateAmountInWords();
        }
    ...
    const withClientRadio = document.getElementById('withClient');
    const withoutClientRadio = document.getElementById('withoutClient');
    const clientSection = document.getElementById('clientSection');

    function toggleClientSection() {
        if (withClientRadio.checked) {
            ...
            document.getElementById('clientState').required = true;
        } else {
            clientSection.style.display = 'none';
            // Make client fields not required
            document.getElementById('clientName').required = false;
            document.getElementById('clientAddress').required = false;
            document.getElementById('clientState').required = false;
            // Clear client fields
    ...
    withClientRadio.addEventListener('change', toggleClientSection);
    withoutClientRadio.addEventListener('change', toggleClientSection);
    toggleClientSection(); // Initial state

    // Modify the form submission to handle both cases
    $('#invoiceForm').submit(function(e) {
        ...
        // Validate client details only if "With Client" is selected
        if (withClientRadio.checked) {
            ...
            if (!document.getElementById('clientAddress').value.trim()) {
                alert('Please enter client address');
                document.getElementById('clientAddress').focus();
                return;
            }
            
            if (!document.getElementById('clientState').value.trim()) {
                alert('Please enter client state');
        ...
        // Add save_invoice parameter and invoice type
        const formData = new FormData(this);
        formData.append('save_invoice', '1');
        formData.append('invoice_type', withClientRadio.checked ? 'with_client' : 'without_client');
        ...
        $.ajax({
            ...
            complete: function(xhr, status) {
                ...
                try {
                    // First check if there's a JSON part in the response (in case HTML was mixed in)
                    ...
                    if (jsonStart >= 0 && jsonEnd > 0) {
                        ...
                        // If no JSON was found, try parsing the whole response
                    ...
                    $('#invoiceForm').prepend(`
                        <div class="alert alert-danger mt-3">
                            <i class="fas fa-exclamation-circle me-2"></i>Error processing response. Please try again.
                        </div>
                    `);
                    ...
                    const showInvoiceSuccessUI = function() {
                        const successHtml = `
                            ...
                            <div class="text-center mt-3">
                                ...
                                <button type="button" class="btn btn-success ms-2" id="sendInvoiceBtn" onclick="sendInvoice(${response.invoice_id}, <?php echo $_SESSION['user_id']; ?>)">
...
function updateAmountInWords() {
    ...
    if (!grandTotal || !amountInWords) {
        console.error('Could not find grandTotal or amountInWords elements');
        return;
    }
    
    const amount = parseFloat(grandTotal.value || 0);
    if (!isNaN(amount)) {
        console.log('Fetching amount in words for:', amount);
        fetch('get_amount_in_words.php?amount=' + amount)
            .then(response => {
                if (!response.ok) {
            ...
            .catch(error => {
                ...
                amountInWords.value = 'Error converting amount';
            });
    }
}

// Call updateAmountInWords when the page loads
document.addEventListener('DOMContentLoaded', function() {
    // Initial calculation
    calculateTotals();
    
    // Add amount to words calculation when grand total changes
    const grandTotal = document.getElementById('grandTotal');
    if (grandTotal) {
        grandTotal.addEventListener('change', updateAmountInWords);
    }
});
</script>
